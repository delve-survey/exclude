...
def fov_geometry(release='y1a1',size=[530,454]):
    """
...
def draw_fov(png,ccds=[],release='y1a1'):
    ...
    ax.imshow(fov,cmap='gray',interpolation='none')
    ax.annotate('exclude',(0.05,0.9),xycoords='axes fraction',ha='left',fontsize=10,
                bbox={'boxstyle':'round','fc':'white'})
    return fov,patches
...
def draw_mini(url):
    ...
    subprocess.check_call('wget %s -nv'%url,shell=True)
    fp = fitsio.read(mini)
